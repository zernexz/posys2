...
class SigmoidLayer : public Layer<FP>{
private:
...
public:
	...
	SigmoidLayer(int in_depth,int in_sx,int in_sy){
...
void set_all_w(vector<FP> aw){
	...
	Vol<FP>* forward(Vol<FP>* V,bool is_training=false){
		...
		//cout << "feed b" << endl;
		//Vol<FP>* V2 = V->clone();
		Vol<FP>* V2 = new Vol<FP>(this->out_sx,this->out_sy,this->out_depth,FP(0.0));
		int N = V2->w.size();
		//cout << "@3" << endl;
		clock_t begin_time = clock();
		//#pragma omp parallel for
		for(int i=0;i<N;i++){
			V2->w[i] = FP(1.0)/(FP(1.0)+exp(FP(-1.0)*V->w[i]));
		}
	...
	void backward(int tmpy=0){
		...
		for(int i=0;i<N;i++){
			FP v2wi = V2->w[i];
			V->dw[i] = v2wi * (FP(1.0) - v2wi) *V2->dw[i];
		}
	}
